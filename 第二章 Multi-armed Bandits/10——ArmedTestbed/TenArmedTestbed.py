...
# @gradientBaseline: if True, use average reward as baseline for gradient based bandit algorithm
    def __init__(self, kArm=10, epsilon=0., initial=0., stepSize=0.1, sampleAverages=False, UCBParam=None,
        ...
        self.indices = np.arange(self.k)     # 指数
        self.time = 0
        ...
        # # of chosen time for each action
        self.actionCount = []
    ...
    def getAction(self):
        ...
        if self.gradient:
            ...
            return np.random.choice(self.indices,p=self.actionProb) # 随机选取
...
# for figure 2.3
def optimisticInitialValue(nBandits,time):
    bandits = [[], []]
...
figure2_1()
epsilonGreedy(20, 10)
optimisticInitialValue(20,20)
ucb(20, 10)
gradientBandit(20,10)
