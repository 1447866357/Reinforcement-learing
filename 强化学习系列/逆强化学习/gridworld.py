...
class Gridworld(object):
    ...
    # off the grid or being blown off the grid. Are we on a corner or not?
        ...
        # Can move off the edge in two directions.
            ...
            Did we intent to move off the grid?"""
            if not (0<=xi +xj<self.grid_size and 0<=yi+yj<self.grid_size):
                return 1-self.wind + self.wind/self.n_actions
            else:
    ...
    def generate_trajectories(self,n_trajectories,trajectory_length,policy,random_start=False):
        ...
        for _ in range(n_trajectories):
            if random_start:
                sx,sy = rn.randint(self.grid_size),rn.randint(self.grid_szie)
            else:
                sx,sy = 0.0
            ...
            for _ in range(trajectory_length):
                ...
                if (0<=sx+action[0]<self.grid_szie and 0<=sy + action[1]<self.grid_szie):
                    next_sx = sx+action[0]
                    next_sy = sy +action[1]
                else:
                    next_sx = sx
                ...
                trajectory_append((state_int,action_int,reward))
